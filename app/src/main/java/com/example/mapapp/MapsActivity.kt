...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    private fun setUpMap() {
        ...
        fusedLocationClient.lastLocation.addOnSuccessListener(this) { location -> //Hakee viimeisen tunnetun sijainnin. Kun tehtävä on suoritettu onnistuneesti, suoritetaan seuraava koodi

            if (location != null){
                lastLocation = location
                val currentLatLong = LatLng(location.latitude, location.longitude)
                placeMarkerOnMap(currentLatLong)
                mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(currentLatLong, 16f))  //siirtää kameran koordinaattejen (LatLong) avulla haluttuun sijaintiin
            }
        }
